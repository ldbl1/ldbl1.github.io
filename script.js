...
function setupContact() {
  ...
  f.addEventListener("submit", async (e) => {
    ...
    if (provider === "formsubmit") {
      ...
      f.action = `https://formsubmit.cloud/${CONFIG.contact.formsubmitToken || CONFIG.contact.email}`;
      f.method = "POST";

      // Evita duplicar campos ocultos si se pulsa varias veces
      f.querySelectorAll(".formsubmit-hidden").forEach((el) => el.remove());
      ...
      const addHiddenInput = (name, value) => {
        ...
        input.name = name;
        input.value = value;
        input.className = "formsubmit-hidden";
        f.appendChild(input);
      };
